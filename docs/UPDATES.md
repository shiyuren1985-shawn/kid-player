...
当前开发版本 0.7.4 / versionCode 15。默认更新清单：
...
1. 完成修改，在 `app/build.gradle` 增加默认 `versionCode`（当前15，下一次至少16）并更新 `versionName`。
...
默认目录：`/Volumes/ZHITAI/AI/KidPlayer/update-server/public/kid-player/`。当前下载路径为 `/kid-player/releases/kid-player-0.7.4-15.apk`。`tools/package.sh` 仅导出本地 APK，不发布清单。
