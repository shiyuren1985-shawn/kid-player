...
public class BiliCatalogTest {
    ...
    private BiliSync.Outcome run(CatalogFixture f)throws Exception{store.prefs.edit().putLong("bili.attempt",0).commit();return BiliSync.run(store,BiliPolicy.UID,true,new BiliClient(f));}
    @Test public void all601UploadsSurviveDiskReloadAndCollectionsDoNotLimitThem()throws Exception{
        ...
        assertEquals(BiliSync.Outcome.UPDATED,run(f));assertEquals(21,f.uploadCalls);
    ...
    @Test public void progressiveSaveRetainsOldCacheAndNeverClaimsPartialSuccess()throws Exception{
        CatalogFixture f=new CatalogFixture();run(f);long prior=store.feed().getLong("syncedAt");f.count=70;f.failPage=2;
        ...
        assertTrue(BiliClient.contains(feed,CatalogFixture.id(70)));assertTrue(BiliClient.contains(feed,CatalogFixture.id(1)));assertTrue(store.syncError().contains("-352"));assertTrue(BiliSync.cooldownMessage(store,BiliPolicy.UID).contains("上次更新未成功"));
    ...
    @Test public void optionalCollectionFailureCannotHideUploads()throws Exception{
        CatalogFixture f=new CatalogFixture();f.denyGroups=true;assertEquals(BiliSync.Outcome.UPDATED,run(f));assertEquals(65,BiliClient.items(store.feed()).size());assertTrue(store.feed().getBoolean("syncComplete"));assertTrue(store.feed().getString("collectionsError").contains("-352"));
    }
