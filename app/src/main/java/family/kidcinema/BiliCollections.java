...
final class BiliCollections {
    ...
    BiliCollections(BiliClient.Transport transport){this.transport=transport;}
    ...
    JSONObject sync() throws Exception {
        ...
        JSONObject lists=BiliClient.data(read(base+"seasons_series_list?mid="+BiliPolicy.UID+"&page_num=1&page_size=20")).getJSONObject("items_lists");
        ...
        for(int i=0;i<seasons.length();i++){
            JSONObject meta=seasons.getJSONObject(i).getJSONObject("meta");BiliPolicy.owner(meta.getLong("mid"));
            ...
            for(int page=1;page<=(total+29)/30;page++){
                JSONObject data=BiliClient.data(read(base+"seasons_archives_list?mid="+BiliPolicy.UID+"&season_id="+season+"&sort_reverse=true&page_num="+page+"&page_size=30"));
                JSONObject identity=data.getJSONObject("meta"),paging=data.getJSONObject("page");BiliPolicy.owner(identity.getLong("mid"));
        ...
        JSONArray videos=new JSONArray();int excluded=0;
        for(String id:sorted){
            ...
            if(video.getJSONObject("owner").getLong("mid")!=BiliPolicy.UID || video.getInt("state")!=0 ||
            ...
            videos.put(new JSONObject().put("bvid",id).put("uid",BiliPolicy.UID).put("title",video.getString("title"))
                .put("author",video.getJSONObject("owner").getString("name")).put("published",published)
                .put("duration",String.format(Locale.ROOT,"%02d:%02d",duration/60,duration%60)));
        }
        return new JSONObject().put("schema",1).put("uid",BiliPolicy.UID).put("syncedAt",System.currentTimeMillis())
            .put("source","public_collections").put("collectionCount",seasons.length()).put("scannedCount",candidates.size())
            .put("excludedCount",excluded).put("videos",videos);
    }
}
